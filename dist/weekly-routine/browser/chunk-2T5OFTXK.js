...
var CATEGORIES = [
  ...
  { name: "Ejercicio", color: "#10b981" },
  { name: "Estudio", color: "#f59e0b" },
  { name: "Personal", color: "#ec4899" },
  { name: "Hogar", color: "#8b5cf6" },
  { name: "Salud", color: "#ef4444" },
  { name: "Ocio", color: "#06b6d4" },
  { name: "Otro", color: "#64748b" }
];
var DAY_NAMES = ["Lunes", "Martes", "Mi\xE9rcoles", "Jueves", "Viernes", "S\xE1bado", "Domingo"];
var DAY_NAMES_SHORT = ["Lun", "Mar", "Mi\xE9", "Jue", "Vie", "S\xE1b", "Dom"];

// ../digital/sesc.flc/node_modules/@angular/forms/fesm2022/forms.mjs
...
function formatWeekRange(mondayDate) {
  ...
  const fromMonth = mondayDate.toLocaleDateString("es-ES", { month: "short" });
  const toMonth = sunday.toLocaleDateString("es-ES", { month: "short" });
...
var DbService = class _DbService {
  ...
  getTasksForWeek(weekStartDate) {
    return __async(this, null, function* () {
      const tasks = yield this.getAll("tasks", "weekStartDate", weekStartDate);
      return tasks.sort((a, b) => a.order - b.order);
    });
  }
  getTasksForDay(weekStartDate, dayOfWeek) {
    return __async(this, null, function* () {
      const tasks = yield this.getAll("tasks", "weekDay", [weekStartDate, dayOfWeek]);
      return tasks.sort((a, b) => a.order - b.order);
    });
  }
  addTask(task) {
    return __async(this, null, function* () {
      yield this.tx("tasks", "readwrite", (store) => store.add(task));
    });
  }
    ...
    return __async(this, null, function* () {
      ...
      return new Promise((resolve, reject) => {
        ...
        getReq.onsuccess = () => {
          ...
          if (existing) {
            const updated = __spreadValues(__spreadValues({}, existing), changes);
  ...
  deleteTask(id) {
    return __async(this, null, function* () {
      yield this.tx("tasks", "readwrite", (store) => store.delete(id));
    });
  }
  toggleTask(id) {
    return __async(this, null, function* () {
  ...
  getRecurringTasks() {
    return __async(this, null, function* () {
      const allTasks = yield this.getAll("tasks");
      return allTasks.filter((t) => t.isRecurring);
    });
  }
  generateWeekFromRecurring(weekStartDate) {
    return __async(this, null, function* () {
      const existing = yield this.getTasksForWeek(weekStartDate);
      if (existing.length > 0)
        return;
      const recurring = yield this.getRecurringTasks();
  ...
  addTemplate(template) {
    return __async(this, null, function* () {
      yield this.tx("templates", "readwrite", (store) => store.add(template));
    });
  }
  deleteTemplate(id) {
    return __async(this, null, function* () {
      yield this.tx("templates", "readwrite", (store) => store.delete(id));
    });
  }
  applyTemplate(template, weekStartDate) {
    return __async(this, null, function* () {
      const db = yield this.getDb();
      const tx = db.transaction("tasks", "readwrite");
      const store = tx.objectStore("tasks");
      for (const t of template.tasks) {
        const task = __spreadProps(__spreadValues({}, t), {
          id: crypto.randomUUID(),
          weekStartDate,
          completed: false
        });
  ...
  getNextOrder(weekStartDate, dayOfWeek) {
    return __async(this, null, function* () {
      const tasks = yield this.getTasksForDay(weekStartDate, dayOfWeek);
      return tasks.length > 0 ? Math.max(...tasks.map((t) => t.order)) + 1 : 0;
    });
  }
  static \u0275fac = function DbService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _DbService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _DbService, factory: _DbService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DbService, [{
    type: Injectable,
...
var WeekService = class _WeekService {
  db;
  currentMonday = signal(getMonday(), ...ngDevMode ? [{ debugName: "currentMonday" }] : []);
  weekStartDate = computed(() => formatWeekStart(this.currentMonday()), ...ngDevMode ? [{ debugName: "weekStartDate" }] : []);
  mondayDate = this.currentMonday.asReadonly();
  tasks = signal([], ...ngDevMode ? [{ debugName: "tasks" }] : []);
  todayDayOfWeek = signal(getTodayDayOfWeek(), ...ngDevMode ? [{ debugName: "todayDayOfWeek" }] : []);
  todayTasks = computed(() => this.tasks().filter((t) => t.dayOfWeek === this.todayDayOfWeek()).sort((a, b) => {
    if (a.time && b.time)
  ...
  todayCompleted = computed(() => this.todayTasks().filter((t) => t.completed).length, ...ngDevMode ? [{ debugName: "todayCompleted" }] : []);
  todayTotal = computed(() => this.todayTasks().length, ...ngDevMode ? [{ debugName: "todayTotal" }] : []);
  todayProgress = computed(() => {
    const total = this.todayTotal();
    return total === 0 ? 0 : this.todayCompleted() / total;
  }, ...ngDevMode ? [{ debugName: "todayProgress" }] : []);
  constructor(db) {
    this.db = db;
  }
  loadWeek() {
    return __async(this, null, function* () {
      ...
      yield this.db.generateWeekFromRecurring(weekStart);
      const tasks = yield this.db.getTasksForWeek(weekStart);
  ...
  updateTask(id, changes) {
    return __async(this, null, function* () {
      yield this.db.updateTask(id, changes);
      yield this.loadWeek();
    });
  }
  deleteTask(id) {
  ...
  getTasksForDay(dayOfWeek) {
    return this.tasks().filter((t) => t.dayOfWeek === dayOfWeek).sort((a, b) => {
      if (a.time && b.time)
        return a.time.localeCompare(b.time);
      if (a.time)
        return -1;
  ...
  getNextOrder(dayOfWeek) {
    return __async(this, null, function* () {
      return this.db.getNextOrder(this.weekStartDate(), dayOfWeek);
    });
  }
  static \u0275fac = function WeekService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _WeekService)(\u0275\u0275inject(DbService));
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _WeekService, factory: _WeekService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(WeekService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], () => [{ type: DbService }], null);
})();
...
@angular/forms/fesm2022/forms.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-2T5OFTXK.js.map
